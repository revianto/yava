// YAVA — Brewing Mode (the hero surface)
// Phases: countdown (3→2→1) → running (multi-session, auto-advance) → complete
// 3 visual variants: focus (dark) · ambient (light hero) · editorial (zine)

const useBrewingEngine = ({ sessions, speed = 1, onComplete }) => {
  // sessions = [{name, duration, note}]
  const [phase, setPhase] = React.useState("countdown");      // countdown | running | paused | complete
  const [countdownN, setCountdownN] = React.useState(3);
  const [sessionIdx, setSessionIdx] = React.useState(0);
  const [elapsedInSession, setElapsedInSession] = React.useState(0);

  // Refs for the rAF loop
  const refStart = React.useRef(null);          // performance.now() baseline for current session
  const refPausedAt = React.useRef(null);
  const refRAF = React.useRef(null);
  const refPhase = React.useRef(phase);
  const refSessionIdx = React.useRef(sessionIdx);
  const refSpeed = React.useRef(speed);
  React.useEffect(() => { refPhase.current = phase; }, [phase]);
  React.useEffect(() => { refSessionIdx.current = sessionIdx; }, [sessionIdx]);
  React.useEffect(() => { refSpeed.current = speed; }, [speed]);

  // 3s countdown — uses real time (no speedup) for the snappy 3-2-1 pop
  React.useEffect(() => {
  ...
  }, [phase === "countdown" ? "yes" : "no"]); // eslint-disable-line

  // rAF loop for session ticking
  React.useEffect(() => {
    if (phase !== "running") return;
    const loop = () => {
      const now = performance.now();
      const dtMs = now - refStart.current;
      const elapsedSec = (dtMs / 1000) * refSpeed.current;
      ...
      if (elapsedSec >= cur.duration) {
        ...
        // Carry the overflow into next session so cumulative time is honored.
        const overflowMs = (elapsedSec - cur.duration) * 1000 / refSpeed.current;
  ...
  }, [phase, sessionIdx, sessions, onComplete]);
...
// ─────────────────────────────────────────────────────────────
// Variant: FOCUS — dark full-bleed, single dominant timer
// ─────────────────────────────────────────────────────────────
const FocusBrewingView = ({ engine, recipe, onExit, speed, sessions }) => {
    ...
    <div className="brewing" style={{ background: "var(--abyss)", color: "#fff", display: "flex", flexDirection: "column" }}>
      ...
      <div className="row between" style={{ padding: "20px 32px" }}>
        ...
        <div className="row gap-2">
          {speed !== 1 && (
            <span className="tag" style={{ background: "rgba(255,255,255,.10)", color: "#fff" }}>
              <span className="live-dot" style={{ background: "var(--lilac)" }} /> Demo {speed}×
            </span>
          )}
          <button className="icon-btn icon-btn--dark" onClick={onExit} title="Keluar">
            <IconClose size={18} />
          </button>
        </div>
      </div>

      {/* Total progress bar — only during run/pause */}
      {phase !== "countdown" && phase !== "complete" && (
        <div style={{ padding: "0 32px 24px" }}>
          ...
          <div className="progress">
            <i style={{ width: `${totalProgress * 100}%` }} />
          </div>
        </div>
      )}
      ...
      <div className="grow row center" style={{ padding: "0 32px" }}>
        {phase === "countdown" && (
          <div className="col center" style={{ alignItems: "center", textAlign: "center" }}>
            ...
            <div
              key={countdownN}
              className="count-pop t-mono-num"
              style={{
                fontSize: 280, fontWeight: 700, lineHeight: 1, color: "var(--coral-red)",
                letterSpacing: "-.04em",
              }}
            >
              {countdownN}
            </div>
            <div className="t-body muted-dark" style={{ marginTop: 24 }}>
              Tarik napas. Tunggu hingga timer mulai.
            </div>
          </div>
        )}
        ...
        {(phase === "running" || phase === "paused") && current && (
          <div className="col center session-enter" key={sessionIdx} style={{ alignItems: "center", textAlign: "center", width: "100%", maxWidth: 880 }}>
            <div className="t-label" style={{ color: "var(--lilac)", marginBottom: 18 }}>
              Sesi {sessionIdx + 1} dari {sessions.length}
            </div>
            <div className="t-display" style={{ fontSize: 72, color: "#fff", marginBottom: 28, letterSpacing: "-.025em" }}>
              {current.name}
            </div>
            <div className="t-mono-num" style={{
              fontSize: 260, fontWeight: 700, lineHeight: .95, letterSpacing: "-.035em",
              color: "var(--powder)",
              opacity: isPaused ? .5 : 1,
            ...
            <div className="t-body muted-dark" style={{ marginTop: 22, maxWidth: 560 }}>
              {current.note || "—"}
            </div>
          </div>
        )}
      ...
      {(phase === "running" || phase === "paused") && (
        <div className="row center gap-2" style={{ padding: "32px 32px 48px" }}>
          <button className="icon-btn icon-btn--dark icon-btn--lg" onClick={reset} title="Reset"><IconReset size={20} /></button>
          {isPaused
            ? <button className="icon-btn icon-btn--lg" onClick={resume} title="Lanjutkan" style={{ background: "var(--coral-red)", color: "#fff", borderColor: "var(--coral-red)" }}><IconPlay size={24} /></button>
            : <button className="icon-btn icon-btn--lg" onClick={pause} title="Jeda" style={{ background: "#fff", color: "var(--deep-ink)", borderColor: "#fff" }}><IconPause size={24} /></button>
          }
          <button className="icon-btn icon-btn--dark icon-btn--lg" onClick={skip} title="Sesi berikutnya"><IconSkip size={20} /></button>
        </div>
      )}

      {/* Next-up preview while running */}
...
// ─────────────────────────────────────────────────────────────
// Variant: AMBIENT — light page, dark timer card, more context
// ─────────────────────────────────────────────────────────────
const AmbientBrewingView = ({ engine, recipe, onExit, speed, sessions }) => {
  const { phase, countdownN, current, remaining, progress, totalProgress, sessionIdx, pause, resume, skip, reset, isPaused } = engine;
  ...
  return (
    <div className="brewing" style={{ background: "var(--lavender-fog)", color: "var(--deep-ink)", display: "flex", flexDirection: "column" }}>
      {/* Top bar */}
      <div className="row between" style={{ padding: "20px 32px" }}>
        <div className="row gap-3"><Logo size={20} /><span className="t-small muted" style={{ marginLeft: 4 }}>{recipe.name}</span></div>
        <div className="row gap-2">
          {speed !== 1 && (
            <span className="tag tag--default"><span className="live-dot" /> Demo {speed}×</span>
          )}
          <button className="icon-btn" onClick={onExit} title="Keluar"><IconClose size={18} /></button>
        </div>
      </div>
      ...
      <div className="grow row center" style={{ padding: "0 48px" }}>
        {phase === "countdown" && (
          <div className="col center" style={{ alignItems: "center", textAlign: "center" }}>
            ...
            <div key={countdownN} className="count-pop t-mono-num" style={{
              fontSize: 320, fontWeight: 700, lineHeight: 1, color: "var(--deep-ink)", letterSpacing: "-.04em",
            }}>{countdownN}</div>
          ...
          <div style={{ width: "100%", maxWidth: 1080, display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 32, alignItems: "stretch" }}>
            ...
            <div className="card card--abyss card--hero session-enter" key={sessionIdx} style={{ padding: 40, color: "#fff", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
              ...
              <div className="col gap-3" style={{ marginTop: 20 }}>
                <div className="t-mono-num" style={{
                  fontSize: 200, fontWeight: 700, lineHeight: 1, color: "var(--powder)", letterSpacing: "-.035em",
                  opacity: isPaused ? .5 : 1, transition: "opacity 200ms",
                }}>{fmt(remaining)}</div>
                <div className="progress"><i style={{ width: `${progress * 100}%` }} /></div>
              </div>
            </div>

            {/* Sidecar: session list + controls */}
            <div className="col gap-3">
                  ...
                  {sessions.map((s, i) => {
                    ...
                    return (
                      <div key={i} className="row gap-3" style={{
                        padding: "12px 0", borderTop: i === 0 ? 0 : "1px solid var(--hairline)", opacity: done ? .45 : 1,
                      }}>
                        <span style={{
                          width: 28, height: 28, borderRadius: "50%",
                          background: active ? "var(--coral-red)" : done ? "var(--deep-ink)" : "transparent",
                          border: active ? "0" : done ? "0" : "1.5px solid var(--deep-ink)",
              ...
              <div className="card" style={{ padding: 20 }}>
                ...
                <div className="row gap-2" style={{ marginTop: 8 }}>
                  ...
                  <button className="icon-btn" onClick={reset} title="Reset"><IconReset size={18} /></button>
                </div>
              </div>
              ...
              <div className="card card--electric" style={{ padding: 18, color: "var(--lilac)" }}>
                ...
                <div className="t-h3" style={{ color: "#fff" }}>
                  {sessions[sessionIdx + 1] ? sessions[sessionIdx + 1].name : "Selesai brewing"}
                </div>
                <div className="t-small" style={{ color: "var(--lilac)" }}>
                  {sessions[sessionIdx + 1] ? `${sessions[sessionIdx + 1].duration}s` : "Nikmati seteguk pertama"}
                </div>
              </div>
            </div>
          </div>
        ...
        {phase === "complete" && (
          <div className="col center" style={{ alignItems: "center", textAlign: "center", maxWidth: 760 }}>
            ...
            <div className="t-display" style={{ fontSize: 88, letterSpacing: "-.03em" }}>
              Selamat menikmati<span style={{ color: "var(--coral-red)" }}>.</span>
            </div>
            <div className="t-body muted" style={{ marginTop: 16, marginBottom: 36 }}>
              {sessions.length} sesi tuntas dalam {fmt(engine.totalDur)}. Diamkan satu menit sebelum sip pertama.
            </div>
            <div className="row gap-2">
              <button className="btn btn--primary btn--lg" onClick={reset}><IconReset size={16} /> Ulangi</button>
      ...
      {/* Bottom progress strip for countdown/running */}
      {(phase === "running" || phase === "paused") && (
...
// ─────────────────────────────────────────────────────────────
// Variant: EDITORIAL — Italian zine. Big asymmetric layout, grid paper, color blocks.
// ─────────────────────────────────────────────────────────────
const EditorialBrewingView = ({ engine, recipe, onExit, speed, sessions }) => {
  const { phase, countdownN, current, remaining, progress, totalProgress, sessionIdx, pause, resume, skip, reset, isPaused } = engine;

  // Big number left, name right, grid paper bg, coral block on side.
  return (
    <div className="brewing grid-paper-bg" style={{ color: "var(--deep-ink)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div className="row between" style={{ padding: "20px 40px" }}>
        ...
        <div className="row gap-2">
          {speed !== 1 && (
            <span className="tag tag--espresso"><span className="live-dot" style={{ background: "#fff" }} /> Demo {speed}×</span>
          )}
          <button className="icon-btn" onClick={onExit} title="Keluar"><IconClose size={18} /></button>
        </div>
      </div>
      ...
      <div className="grow" style={{ position: "relative", padding: "0 40px" }}>
          ...
          <div style={{ height: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", alignItems: "center", gap: 48 }}>
            <div>
              ...
              <div className="t-display" style={{ fontSize: 96, letterSpacing: "-.035em", lineHeight: .95 }}>
                Tarik napas.<br />Mulai dari{" "}
                <span style={{ color: "var(--coral-red)" }}>aroma</span>.
              </div>
              <div className="t-body muted" style={{ marginTop: 16, maxWidth: 480 }}>
                Tiga detik untuk hadir. Tidak lebih, tidak kurang.
              </div>
            </div>
            <div className="col center" style={{ alignItems: "center", padding: 40 }}>
              <div key={countdownN} className="count-pop t-mono-num" style={{
                fontSize: 460, fontWeight: 700, lineHeight: 1, color: "var(--coral-red)",
                letterSpacing: "-.05em",
              }}>{countdownN}</div>
            </div>
          </div>
        ...
        {(phase === "running" || phase === "paused") && current && (
          <div key={sessionIdx} className="session-enter" style={{
            display: "grid", gridTemplateColumns: "1fr 1.2fr",
            height: "100%", alignItems: "stretch", gap: 0, position: "relative",
          }}>
            {/* Left column: session name, note, controls */}
            <div className="col" style={{ padding: "24px 0", justifyContent: "space-between" }}>
              <div>
                ...
                <div className="t-body" style={{ marginTop: 18, maxWidth: 460, fontSize: 17 }}>
                  {current.note}
                </div>
              </div>

              <div>
                {/* Session steps mini-flow */}
                <div className="row gap-1" style={{ marginBottom: 20, flexWrap: "wrap" }}>
                  {sessions.map((s, i) => (
                    <div key={i} style={{
                      height: 6, flex: 1, minWidth: 40, borderRadius: 99,
                      background: i < sessionIdx ? "var(--deep-ink)" : i === sessionIdx ? "var(--coral-red)" : "rgba(26,21,48,.15)",
                      position: "relative", overflow: "hidden",
                    }}>
                      {i === sessionIdx && (
                        <div style={{
                          position: "absolute", inset: 0, background: "var(--deep-ink)",
                          width: `${progress * 100}%`, transition: "width 100ms linear",
                        }} />
                      )}
                    </div>
                  ))}
                ...
                <div className="row gap-2">
                  ...
                  <button className="icon-btn icon-btn--lg" onClick={reset} title="Reset"><IconReset size={20} /></button>
                </div>
                <div className="t-small muted" style={{ marginTop: 16 }}>
                  Berikutnya: <strong style={{ color: "var(--deep-ink)" }}>
                    {sessions[sessionIdx + 1] ? sessions[sessionIdx + 1].name : "selesai — diamkan satu menit"}
                  </strong>
                </div>
              </div>
            </div>

            {/* Right column: massive timer in coral block */}
            <div style={{
              position: "relative",
              ...
              <div className="t-mono-num" style={{
                fontSize: 360, fontWeight: 700, lineHeight: .85, letterSpacing: "-.04em",
                textAlign: "center", opacity: isPaused ? .5 : 1, transition: "opacity 200ms",
              }}>
                {fmt(remaining)}
              </div>
              <div>
        ...
        {phase === "complete" && (
          <div style={{ height: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", alignItems: "center", gap: 48 }}>
            <div>
              ...
              <div className="t-display" style={{ fontSize: 120, letterSpacing: "-.035em", lineHeight: .9 }}>
                Selamat<br />menikmati<span style={{ color: "var(--coral-red)" }}>.</span>
              </div>
              <div className="t-body muted" style={{ marginTop: 20, maxWidth: 520, fontSize: 17 }}>
                {sessions.length} sesi tuntas dalam {fmt(engine.totalDur)}. Diamkan satu menit sebelum sip pertama — biarkan aroma menetap.
              </div>
...
// Root brewing controller — picks the variant
const BrewingMode = ({ recipeId, onExit, variant = "focus", speed = 1 }) => {
  const recipe = window.YAVA_DATA.RECIPES.find((r) => r.id === recipeId);
  const sessions = React.useMemo(
    () => (recipe ? recipe.timeline.filter((s) => s.kind === "session") : []),
    [recipeId]
  );

  const engine = useBrewingEngine({ sessions, speed });
  ...
  const props = { engine, recipe, onExit, speed, sessions };
  if (variant === "ambient") return <AmbientBrewingView {...props} />;
  if (variant === "editorial") return <EditorialBrewingView {...props} />;
  return <FocusBrewingView {...props} />;
};

window.BrewingMode = BrewingMode;
